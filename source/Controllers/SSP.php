...
class SSP {
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				if ( isset( $column['formatter'] ) ) {
                    ...
                    if(!empty($column['db'])){
						// Adicionei essa conversao pois o json não funciona com acentos
						$data_utf8_encode = utf8_encode($data[$i][ $columns[$j]['db'] ]);
                        $row[ $column['dt'] ] = $data_utf8_encode;
                    }
                    else{
                        $row[ $column['dt'] ] = "";
                    }
			...
			// Converter para UTF-8 encode, pois json não aceita acentos
			// var_dump($row);
			$out[] = $row;
		}
		return $out;
	...
	static function complex (
		...
		// Main query to actually get the data
		// Modificado o parametro da query abaixo para aceitar join
